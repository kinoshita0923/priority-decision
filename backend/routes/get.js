...
router.get('/', async (req, res) => {
    ...
    // ユーザのタスクを取得
    let sql =
        'SELECT * FROM tasks inner join relations on tasks.task_id = relations.task_id join genres on genres.genre_id = relations.genre_id where relations.user_id = ? and relations.deleted != 1 ORDER BY relations.deadline ASC;';
    ...
    connection.beginTransaction((err) => {
        ...
        connection.query(sql, values, (err, results) => {
            ...
            for (let genre in genres_list) {
                sql =
                    'SELECT start_date, end_date, genre_id from relations where user_id = ? and end_date IS NOT NULL and start_date IS NOT NULL and genre_id = ?;';
            ...
            let importantTaskId;
            sql =
                'SELECT * FROM tasks natural join relations WHERE user_id = ? and deleted = ?;';
            values = [user_id, 0];
            connection.query(sql, values, (err, results) => {
                if (err) {
                    return connection.rollback(() => {
