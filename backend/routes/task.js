...
router.post('/add/', async (req, res) => {
    ...
    connection.beginTransaction((err) => {
        ...
        connection.query(sql, values, (err, results) => {
            ...
            sql = 'INSERT INTO relations(user_id, task_id, start_date, end_date, deadline, status, genre_id, deleted) VALUES(?, NULL, NULL, NULL, ?, ?, ?, ?);';
            values = [user_id, req.body.deadline, false, req.body.genre_id, false];
            connection.query(sql, values, (err, results) => {
                if (err) {
                    return connection.rollback(() => {
                        res.send(err);
...
router.post('/edit_status/', async (req, res) => {
    ...
    if (status == 1) {
        sql = 'UPDATE relations SET status = ?, end_date = ? WHERE task_id = ? and user_id = ?;';
        let currentDate = new Date();
        values = [req.body.status, currentDate, req.body.task_id, user_id];
    } else {
        sql = 'UPDATE relations SET status = ? WHERE task_id = ? and user_id = ?;';
...
router.post('/start_date/', async (req, res) => {
    ...
    let user_id = jwt.verify(req.cookies.token, process.env.jwt_password).user_id;
    let sql = 'UPDATE relations SET start_date = ? WHERE task_id =? and user_id = ?;';
    let currentDate = new Date();
...
router.post('/delete/', async (req, res) => {
    ...
    // 削除したタスクはdeletedを1に
    let sql = 'UPDATE relations SET deleted = ? WHERE task_id = ? and user_id = ?;';
    let user_id = jwt.verify(req.cookies.token, process.env.jwt_password).user_id;
