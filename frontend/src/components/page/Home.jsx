...
const Home = () => {
    ...
    const updateTaskStatus = (data, status) => {
        ...
        axios
            .post('/api/task/edit_status/', params, {
                withCredentials: true,
            })
            .then((res) => {
                loadMore();
    ...
    const startTask = (task_id) => {
        ...
        axios
            .post('/api/task/start_date', params, {
                withCredentials: true,
            })
            .then((res) => {
                loadMore();
    ...
    return (
        <React.StrictMode>
            <div className={css.task_list}>
                {initialNetwork < 1 || importantTask.length === 0 ? null : (
                    <div className={css.tasks_title}>優先順位の高いタスク</div>
                )}
                {initialNetwork < 1 || importantTask.length == 0 ? null : (
                    <Task
                        key={importantTask.task_id}
                        name={importantTask.task_name}
                        deadline={changeListDeadline(importantTask.deadline)}
                        status={importantTask.status}
                        task_id={importantTask.task_id}
                        start_date={importantTask.start_date}
                ...
                {initialNetwork < 1 || list.length == 0 ? null : (
                    <div className={css.tasks_title}>タスク</div>
                )}
                <ul>
                    {initialNetwork < 1 || list.length == 0
                        ? null
                        : Object.keys(list).map((key) => {
                              return (
                                  <li key={list[key].task_id}>
                                      <Task
                                          name={list[key].task_name}
                                          deadline={changeListDeadline(
                                              list[key].deadline
                                          )}
                                          status={list[key].status}
                                          task_id={list[key].task_id}
                                          start_date={list[key].start_date}
                                          updateTaskStatus={updateTaskStatus}
                                          deleteTask={deleteTask}
                                          startTask={startTask}
                                      />
            ...
            <AddButton
                genres={genres}
                addGenre={addGenre}
                changeAddGenre={changeAddGenreText}
